...
# --- Obuna tekshirish (Xatolikka chidamli) ---
def check_sub(user_id):
    try:
        member = bot.get_chat_member(CHANNEL_USERNAME, user_id)
        return member.status in ["creator", "administrator", "member"]
    except: return True # Xatolik bo'lsa o'tkazib yuboradi
...
def process_yt_link(message, text):
    ...
    try:
        # Extractor args YouTube cheklovlarini bir oz yumshatadi
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🎵 MP3", callback_data="type_mp3_0"),
...
def search_youtube(message):
    ...
    except Exception: bot.edit_message_text("❌ Qidiruvda xatolik.", message.chat.id, status.message_id)
...
def handle_callbacks(call):
    try:
        if call.data == "check_sub":
            ...
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("🎵 MP3", callback_data=f"type_mp3_{idx}"),
                       InlineKeyboardButton("🎬 MP4", callback_data=f"type_mp4_{idx}"))
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=markup)
        
        elif call.data.startswith("type_"):
            download_yt(call)
    except Exception: pass # O'chirishda xato bo'lsa ham bot to'xtab qolmaydi
...
def download_yt(call):
    ...
    try:
        ...
        with open(real_file, 'rb') as f:
            ...
            else: bot.send_video(call.message.chat.id, v, caption=AD_TEXT)
        
        # Xabarni o'chirish (Xatoga chidamli)
        ...
        bot.send_message(call.message.chat.id, f"❌ Yuklashda xato. Internet sekinligi yoki YouTube cheklovi.")
...
if __name__ == "__main__":
    ...
    # Timeoutni oshiramiz (20 dan 90 ga)
